...
var import_Cesium2 = __toESM(require_Cesium(), 1);

// Specs/equals.js
var import_Cesium = __toESM(require_Cesium(), 1);
function isTypedArray(o) {
  return import_Cesium.FeatureDetection.typedArrayTypes.some(function(type) {
...
// Specs/addDefaultMatchers.js
var import_Cesium3 = __toESM(require_Cesium(), 1);
      ...
      return {
        compare: function(actual, expected, epsilon) {
          function equalityTester(a, b) {
            ...
            if ((0, import_Cesium2.defined)(a) && (0, import_Cesium2.defined)(b)) {
    ...
    toPickAndCall: function(util) {
      return {
        compare: function(actual, expected, args) {
          const scene = actual;
          const result = scene.pick((0, import_Cesium2.defaultValue)(args, new import_Cesium2.Cartesian2(0, 0)));
          const webglStub2 = !!window.webglStub;
          if (!webglStub2) {
            const callback = expected;
            callback(result);
    ...
    toDrillPickAndCall: function(util) {
      return {
        compare: function(actual, expected, limit) {
          const scene = actual;
          const pickedObjects = scene.drillPick(new import_Cesium2.Cartesian2(0, 0), limit);
          const webglStub2 = !!window.webglStub;
          if (!webglStub2) {
            const callback = expected;
    ...
    toPickPositionAndCall: function(util) {
      return {
        compare: function(actual, expected, x, y) {
          ...
          x = (0, import_Cesium2.defaultValue)(x, canvas.clientWidth / 2);
          y = (0, import_Cesium2.defaultValue)(y, canvas.clientHeight / 2);
          const result = scene.pickPosition(new import_Cesium2.Cartesian2(x, y));
          const webglStub2 = !!window.webglStub;
          if (!webglStub2) {
            const callback = expected;
            callback(result);
    ...
    toReadPixels: function(util) {
      return {
        compare: function(actual, expected) {
          ...
          if ((0, import_Cesium2.defined)(options.context)) {
            context = options.context;
            framebuffer = options.framebuffer;
            epsilon = (0, import_Cesium2.defaultValue)(options.epsilon, epsilon);
          } else {
            context = options;
          }
          ...
          const webglStub2 = !!window.webglStub;
          if (!webglStub2) {
            if (!import_Cesium3.Math.equalsEpsilon(rgba[0], expected[0], 0, epsilon) || !import_Cesium3.Math.equalsEpsilon(rgba[1], expected[1], 0, epsilon) || !import_Cesium3.Math.equalsEpsilon(rgba[2], expected[2], 0, epsilon) || !import_Cesium3.Math.equalsEpsilon(rgba[3], expected[3], 0, epsilon)) {
              pass = false;
              if (epsilon === 0) {
...
function renderAndReadPixels(options) {
  let scene;
  if ((0, import_Cesium2.defined)(options.scene)) {
    scene = options.scene;
    const time = options.time;
    scene.initializeFrame();
    if ((0, import_Cesium2.defined)(options.primeShadowMap)) {
      scene.render(time);
    }
    scene.render(time);
  } else {
...
function isTypedArray2(o) {
  return import_Cesium2.FeatureDetection.typedArrayTypes.some(function(type) {
    return o instanceof type;
  });
}
...
function pickPrimitiveEquals(actual, expected, x, y, width, height) {
  const scene = actual;
  const windowPosition = new import_Cesium2.Cartesian2(x, y);
  ...
  let pass = true;
  let message;
  if ((0, import_Cesium2.defined)(expected)) {
    pass = result.primitive === expected;
  } else {
    pass = !(0, import_Cesium2.defined)(result);
  }
  if (!pass) {
...
function contextRenderAndReadPixels(options) {
  ...
  const depth = (0, import_Cesium2.defaultValue)(options.depth, 0);
  const clear = (0, import_Cesium2.defaultValue)(options.clear, true);
  let clearColor;
  if (!(0, import_Cesium2.defined)(context)) {
    throw new import_Cesium2.DeveloperError("options.context is required.");
  }
  if (!(0, import_Cesium2.defined)(fs) && !(0, import_Cesium2.defined)(sp)) {
    throw new import_Cesium2.DeveloperError(
  ...
  if ((0, import_Cesium2.defined)(vs) && (0, import_Cesium2.defined)(sp)) {
    throw new import_Cesium2.DeveloperError(
      "Both options.vertexShader and options.shaderProgram can not be used at the same time."
    );
  }
  if (!(0, import_Cesium2.defined)(sp)) {
    if (!(0, import_Cesium2.defined)(vs)) {
      vs = "attribute vec4 position; void main() { gl_PointSize = 1.0; gl_Position = position; }";
    }
    sp = import_Cesium2.ShaderProgram.fromCache({
      context,
  ...
  let va = new import_Cesium2.VertexArray({
    context,
    attributes: [
      {
        index: 0,
        vertexBuffer: import_Cesium2.Buffer.createVertexBuffer({
          context,
          typedArray: new Float32Array([0, 0, depth, 1]),
          usage: import_Cesium2.BufferUsage.STATIC_DRAW
        }),
  ...
  if (clear) {
    import_Cesium2.ClearCommand.ALL.execute(context);
    clearColor = context.readPixels();
  }
  const command = new import_Cesium2.DrawCommand({
    primitiveType: import_Cesium2.PrimitiveType.POINTS,
    shaderProgram: sp,
    vertexArray: va,
    uniformMap,
    modelMatrix
...
function expectContextToRender(actual, expected, expectEqual) {
  ...
  const context = options.context;
  const clear = (0, import_Cesium2.defaultValue)(options.clear, true);
  const epsilon = (0, import_Cesium2.defaultValue)(options.epsilon, 0);
  if (!(0, import_Cesium2.defined)(expected)) {
  ...
  if (!webglStub2) {
    ...
    } else if (import_Cesium3.Math.equalsEpsilon(rgba[0], expected[0], 0, epsilon) && import_Cesium3.Math.equalsEpsilon(rgba[1], expected[1], 0, epsilon) && import_Cesium3.Math.equalsEpsilon(rgba[2], expected[2], 0, epsilon) && import_Cesium3.Math.equalsEpsilon(rgba[3], expected[3], 0, epsilon)) {
      return {
        pass: false,
        message: `Expected context not to render ${expected}, but rendered: ${rgba}`
      };
...
var import_Cesium4 = __toESM(require_Cesium(), 1);
function equalsMethodEqualityTester(a, b) {
  let to_run;
  if (a !== null && (0, import_Cesium4.defined)(a)) {
  ...
  if (b !== null && (0, import_Cesium4.defined)(b)) {
